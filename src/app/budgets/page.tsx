...
export default function BudgetsPage() {
  ...
  const budgetedCats = new Set(data?.lines.map((l) => l.category) ?? []);
    ...
    <div>
      ...
      {data && data.lines.length > 0 && (
        <div className="mb-6 grid gap-4 sm:grid-cols-4">
          ...
          <div className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
            ...
            <p className="text-xl font-bold text-gray-900">{formatCurrency(data.totalActual)}</p>
          </div>
          <div className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
            <p className="text-xs text-gray-500">Dépassements</p>
            <p className={cn("text-xl font-bold", data.overCount > 0 ? "text-red-600" : "text-emerald-600")}>
              {data.overCount}
            </p>
          </div>
          <div className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
            <p className="text-xs text-gray-500">Hors budget</p>
            <p className={cn("text-xl font-bold", data.unbudgetedSpend > 0 ? "text-amber-600" : "text-gray-900")}>
              {formatCurrency(data.unbudgetedSpend)}
            </p>
          </div>
        </div>
      )}
      ...
      {loading ? (
        ...
        <>
          ...
          {data && data.unbudgetedSpend > 0 && data.lines.length > 0 && (
            <div className="flex items-start gap-2 rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-700">
              <AlertTriangle className="mt-0.5 h-4 w-4 flex-shrink-0" />
              <span>
                {formatCurrency(data.unbudgetedSpend)} dépensés dans des catégories sans budget ce mois-ci.
              </span>
            </div>
          )}
        </>
